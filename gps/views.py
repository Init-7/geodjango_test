        ...
        if(TrabajadorDevice.objects.filter(trabajador=t.id).exists()):
            ...
            if(Zona.objects.filter(zona__bbcontains=tp).exists()):
                ...
                if (z != t.last_z):
                    ...
                    msg = "AVISO: Trabajador %s %s Ingreso a zona: %s Nivel riesgo: %s Supervisor: %s %s %s. Monitorear en: http://www.staff.estchile.cl/gps/sms/%s" % ('NOMBRE', t.apellidop, z, t.nivel_riesgo, 'NAME', t.supervisor.apellidop, t.supervisor.fono, td.fono_gps)
                    sms_twilio_z(msg)
                    t.last_z = z
                else:
